...
/**
  ...
  *
  */
class MicroServices[F[_] : Effect : cats.Functor] {
...
object MicroServices {
  ...
  //import cats.implicits._
  import cats.{ Monad }
  import cats._//, cats.instances.all._, cats.syntax.flatMap._

  def apply[F[_] : Effect : Monad : RecursiveTailRecM]: MicroServices[F] = new MicroServices[F]

  implicit def instance[F[_] : Effect : Monad : RecursiveTailRecM]: MicroServices[F] = apply[F]
}
...
object Runner extends App {
  ...
  for {
    a <- MicroServices[Future].fetchOptionalUser(101l)
    b <- MicroServices[Future].fetchAddress(24l)
  } yield println(a,b)
}